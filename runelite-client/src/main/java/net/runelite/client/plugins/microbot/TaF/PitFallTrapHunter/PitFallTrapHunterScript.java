...
public class PitFallTrapHunterScript extends Script {
    ...
    private static void WalkBackToHunterArea(PitFallTrapHunterConfig config, PitFallTrapHunterPlugin plugin) {
    ...
    public boolean run(PitFallTrapHunterConfig config, PitFallTrapHunterPlugin plugin) {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                PitFallTrapHunting pitFallTrapHunting = getDeadFallHunting(config);
                if (pitFallTrapHunting == null) {
                    return;
                }

                // Walk to area if not nearby
                if (!isNearArea(pitFallTrapHunting)) {
                    Microbot.log("Walking to " + pitFallTrapHunting.getName() + " hunting area");
                    Rs2Walker.walkTo(pitFallTrapHunting.getHuntingPoint());
                    return;
                }
                ...
                if (activeTrapCount < 1 && Rs2Inventory.hasItem("logs")) {
                    if (!Rs2Player.isAnimating()) {
                        setNewTrap(pitFallTrapHunting, config);
                    }
                } else {
    ...
    private PitFallTrapHunting getDeadFallHunting(PitFallTrapHunterConfig config) {
    ...
    private PitFallTrapHunting getBestDeadFallHuntingCreature() {
        var skillLevel = Microbot.getClient().getRealSkillLevel(Skill.HUNTER);
        if (skillLevel > 90) {
            return PitFallTrapHunting.MOONLIGHT_ANTELOPE;
        } else if (skillLevel > 71) {
            return PitFallTrapHunting.SUNLIGHT_ANTELOPE;
        } else if (skillLevel > 54) {
            return PitFallTrapHunting.SABRE_TOOTHED_KYATT;
        } else if (skillLevel > 40) {
            return PitFallTrapHunting.HORNED_GRAAHK;
        } else if (skillLevel > 30) {
            return PitFallTrapHunting.SPINED_LARUPIA;
        }
    ...
    private boolean isNearArea(PitFallTrapHunting pitFallTrapHunting) {
        WorldPoint currentLocation = Rs2Player.getWorldLocation();
        return currentLocation.distanceTo(pitFallTrapHunting.getHuntingPoint()) <= 50;
    }

    private boolean handleExistingTraps(PitFallTrapHunterPlugin plugin, PitFallTrapHunterConfig config) {
    ...
    private void setNewTrap(PitFallTrapHunting pitFallTrapHunting, PitFallTrapHunterConfig config) {
        if (Rs2GameObject.exists(pitFallTrapHunting.getTrapId())) {
            Rs2GameObject.interact(pitFallTrapHunting.getTrapId(), "Set-trap");
